...
const Navbar = () => {
  ...
  const address = authenticated ? user?.wallet?.address : wagmiAddress;
  ...
  return (
    <div className="h-[64px] font-['Satoshi',sans-serif] border-b border-white/10 px-6 flex items-center justify-between relative bg-[#0a0d14]">
      ...
      {/* Wallet / Connect */}
      {isUserConnected && globalData.token ? (
          ...
          <Popover>
            ...
            >
              <div className="flex flex-col gap-1">
                <button
                  onClick={() => {
                    dispatch(clearChat({network: network?.id, address: wagmiAddress!}));
                    navigate("/chat");
                  }}
