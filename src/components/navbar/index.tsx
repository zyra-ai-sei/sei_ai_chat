...
const Navbar = () => {
  ...
  return (
    <div className="h-[64px] font-['Satoshi',sans-serif] border-b border-white/10 px-6 flex items-center justify-between relative bg-[#0a0d14]">
      ...
      <nav className="items-center h-full gap-1 hidden md:flex">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          className="md:hidden flex p-2 box-content text-[#fff] rounded-full cursor-pointer hover:bg-white/10 transition-colors"
      ...
      {isMobileMenuOpen && (
        <nav className="absolute top-[64px] left-0 w-full bg-[#0a0d14] border-b border-white/10 flex flex-col md:hidden z-[100]">
          {routes.map((route, index) => {
            const isActive = location.pathname === route.url;
            return (
              <Link
                ...
                className={`
                  relative py-4 px-6 text-sm font-medium tracking-wide border-l-4
                  transition-colors duration-200
                  ${isActive ? "text-white border-blue-500 bg-white/5" : "text-white/50 border-transparent hover:text-white/80 hover:bg-white-[0.02]"}
                `}
              >
                {route.name}
              </Link>
